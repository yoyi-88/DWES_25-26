...
class libroModel extends Model {
    ...
    */
    ...
    /*
        Método: get_csv()
        Descripción: Obtiene los libros en formato apto para exportar/importar
    */
    public function get_csv() {
        try {
            $sql = "SELECT titulo, autor_id, editorial_id, precio_venta, stock, fecha_edicion, isbn FROM libros";
            $geslibros = $this->db->connect();
            $stmt = $geslibros->prepare($sql);
            $stmt->setFetchMode(PDO::FETCH_ASSOC);
            $stmt->execute();
