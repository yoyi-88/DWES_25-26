                ...
                <tbody>
                    <?php foreach ($articulos as $articulo) : ?>
                        <tr>
                            ...
                            <td><?= implode(", ", Class_tabla_articulos::categorias_indices_a_nombres($articulo->getCategorias())); ?></td>
                            <td class="text-end"><?= number_format($articulo->getUnidades(), 2, ',', '.'); ?></td>
                            <td class="text-end"><?= number_format($articulo->getPrecio(), 2, ',', '.'); ?></td>
                            <td>
                                <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                    <a href="edit.php?id=<?= $articulo->getId(); ?>" class="btn btn-warning btn-sm" title="Editar">
                                        <i class="bi bi-pen"></i>
                                    </a>
                                    <a href="delete.php?id=<?= $articulo->getId(); ?>" class="btn btn-danger btn-sm" title="Eliminar"
                                        onclick="return confirm('Confirmar eliminación del artículo')">
                                        <i class="bi bi-trash"></i>
                                    </a>
                                    <a href="show.php?id=<?= $articulo->getId(); ?>" class="btn btn-primary btn-sm" title="Ver">
                                        <i class="bi bi-eye"></i>
                                    </a>
