...
class Alumno extends Controller
{
    ...
    */
    public function export($param = [])
    {
        ...
        $this->requirePrivilege($GLOBALS['alumno']['export']);
    ...
    /*
        ...
        url asociada: /alumno/import/csv
    ...
    */
    public function import($param = [])
    {
        ...
        // Comprobar si el usuario tiene permisos
        $this->requirePrivilege($GLOBALS['alumno']['import']);
        ...
        // Cargar la vista
        $this->view->render('alumno/import/index');
    }

    /*
        Método validar()

        Descripción: Valida y procesa un archivo CSV para importar alumnos

        url asociada: /alumno/validar/csv
    ...
    */
    public function validar($param = [])
    {
        ...
        if ($extension !== 'csv') {
            ...
            $this->errorcsv("El archivo debe tener formato CSV");
        }
        ...
        while (($linea = fgetcsv($fichero, 0, ';')) !== FALSE) {
            ...
            // Validar DNI
            if (!$this->model->validate_unique_dni($linea[5])) {
                $mensaje_error = 'En la línea'. count($alumnos). ' del fichero, el DNI ' . $linea[5] . ' ya existe';
                $this->errorcsv($mensaje_error);
            }

            // Validar email
            if (!$this->model->validate_unique_email($linea[2])) {
                $mensaje_error = 'En la línea'. count($alumnos). ' del fichero, el email ' . $linea[2] . ' ya existe';
                $this->errorcsv($mensaje_error);
            }

            // Validar curso_id
            if (!$this->model->validate_curso_exists($linea[7])) {
                $mensaje_error = 'En la línea'. count($alumnos). ' del fichero, el curso ' . $linea[7] . ' no existe';
                $this->errorcsv($mensaje_error);
            }
        }
        ...
        $_SESSION['notify'] = $count . ' Alumnos importados con éxito';
    ...
    /*
        Método: errorcsv()
        Descripción: Muestra un mensaje de error al importar un archivo CSV y 
        redirige al formulario de importación
    */
    private function errorcsv($mensaje)
    {
        // Genero mensaje de error
        $_SESSION['error'] = $mensaje;
    
        // redireciona al formulario de importación
        header('location:' . URL . 'alumno/import/csv/');
        exit();
    }
